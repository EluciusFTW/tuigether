...
let private normalBindings: KeyBinding<Model, Msg> list = [
  ...
  KeyBinding.dynamic (CharKey 'c') (fun model ->
    ...
    | false -> {
        Description = "switch branch (wrong repo)"
        Message = None
      })
...
let private switchChoosingBindings: KeyBinding<Model, Msg> list = [
  KeyBinding.createSpecial ConsoleKey.UpArrow "up" SwitchSelectUp
  KeyBinding.createSpecial ConsoleKey.DownArrow "down" SwitchSelectDown
  KeyBinding.createSpecial ConsoleKey.Enter "switch" ChooseBranch
  KeyBinding.createSpecial ConsoleKey.Escape "cancel" DismissSwitchPopup
]
...
let handleKey (key: ConsoleKeyInfo) (model: Model) : Msg option =
  ...
  | Normal ->
    ...
    | 'c' when isRepoOK model -> Some BeginSwitchBranch
  ...
  | SwitchPopup(ChoosingBranch _) ->
    ...
    | _ -> None
  | SwitchPopup(ConfirmDirty _) ->
    match key.Key with
    | ConsoleKey.Escape -> Some DismissSwitchPopup
    | _ ->
      match key.KeyChar with
      | 's' -> Some(SwitchWithPolicy Git.StashAndCarry)
      | 'l' -> Some(SwitchWithPolicy Git.StashAndLeave)
      | _ -> None
  | SwitchPopup(SwitchFailed _)
  | SwitchPopup(SwitchedWithNotes _) ->
    match key.Key with
    | ConsoleKey.Enter
    | ConsoleKey.Escape -> Some DismissSwitchPopup
    | _ -> None
...
let keyMap (model: Model) =
  let bindings =
    ...
    | SwitchPopup LoadingBranches
    | SwitchPopup(SwitchingTo _) -> emptyBindings
    | SwitchPopup(ChoosingBranch _) -> switchChoosingBindings
    | SwitchPopup(ConfirmDirty _) -> switchDirtyBindings
    | SwitchPopup(SwitchFailed _)
    | SwitchPopup(SwitchedWithNotes _) -> switchDoneBindings
...
// rows tall — so SessionView renders this as a work-area overlay instead, where the
// branch list has room to breathe.
let switchOverlay (model: Model) : IWidget option =
  match model.InputMode with
  | SwitchPopup stage ->
    Some(
      { new IWidget with
...
let widget (model: Model) : IWidget =
  { new IWidget with
      member _.Render(ctx) =
        ...
        // The branch picker is drawn by SessionView as a work-area overlay, since a
